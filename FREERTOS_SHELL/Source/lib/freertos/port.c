...
static void prvSetupTimerInterrupt( void )
{
	...
	#elif (defined TCC1_CCA_vect)  // Xmega Board Timer C0
		#define RT_VECT TCC1_CCA_vect
		TCC1_CCAH = compare_match.byte[1];      // Set high byte of compare match
		TCC1_CCAL = compare_match.byte[0];      // register, then low byte
		TCC1_CTRLA = (1 << TC0_CLKSEL0_bp);		// Prescale F_CPU / 1,
		TCC1_CTRLB = (1 << TC0_CCAEN_bp | 1 << TC0_WGMODE0_bp);		// Enable Compare Capture A
		TCC1_INTCTRLB = (0 << TC0_CCAINTLVL1_bp | 1 << TC0_CCAINTLVL0_bp);
	#else
